...
const SearchStops = ({ navigation }) => {
    ...
    const fetchStops = () => {
        ...
        if (Platform.OS != "android") {
            ...
            pinningFetch("https://openapi.izmir.bel.tr/api/eshot/durakara/" + encodeURI(val), { method: 'GET', timeoutInterval: 10000, sslPinning: { certs: ['mycert'] }, })
                .then((response) => response.json())
                .then((responseJson) => {
                    console.log(responseJson)
                    for (let key in responseJson["Duraklar"]) {
    ...
    const renderItem = ({ item }) => (
        <View style={{ backgroundColor: "#181818", padding: 15, margin: 15, borderRadius: 6, }}>
            ...
            <View style={{ margin: 15, backgroundColor: "#212121", padding: 5, borderRadius: 6, borderColor: "#262626", borderWidth: 1 }}>
                <Text style={{ fontWeight: 'bold', fontSize: 15, textAlign: 'center', color: "#ebdbb2" }}>Geçen hatlar:</Text>
                <Text style={{ color: "#d3869b", textAlign: 'center', fontWeight: '500', marginTop: 5 }}>• {item.gecenHatlar.join("\n• ")}</Text>
            </View>
        </View>
    );
    ...
    return (
        <SafeAreaView style={Styles.safeAreaView}>
            <TextInput placeholder='Durak adi veya numarasi' style={
                { backgroundColor: "#161616", color: "#ebdbb2", borderColor: "#282828", borderWidth: 1, width: "60%", padding: 15, margin: 5, marginTop: 35 }} onChangeText={(text) => { setVal(text); setDurakListe([]) }} value={val} onFocus={() => setDurakListe([])} />

            <Text style={{color:"#ebdbb2", padding:30}}>{val.length >= 3 ? "" : "Arama butonu durak adı veya numarası girene kadar erişilemez."}</Text>
